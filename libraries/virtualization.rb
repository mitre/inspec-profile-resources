  ...
  module Resources
    class Virtualization < Inspec.resource(1)
      ...
      def detect_xen
        ...
        @virtualization_data[:role] = if inspec.file('/proc/xen/capabilities').content =~ /control_d/i
                                        'host'
                                      else
                                        'guest'
                                      end
      ...
      def detect_kvm_from_sys
        ...
        @virtualization_data[:system] = 'kvm'
        @virtualization_data[:role] = if inspec.file('/proc/cpuinfo').content =~ /hypervisor/
                                        'guest'
                                      else
                                        'host'
                                      end
      ...
      def detect_linux_vserver
        ...
        @virtualization_data[:role] = if vxid[2] == '0'
                                        'host'
                                      else
                                        'guest'
                                      end
      ...
      def detect_container
        ...
        if cgroup_content =~ %r{^\d+:[^:]+:/(lxc|docker)/.+$} ||
          ...
          @virtualization_data[:system] = ::Regexp.last_match(1)
          @virtualization_data[:role] = 'guest'
      ...
      def detect_vmware
        ...
        return false unless product_name =~ /^VMware/
      ...
      def detect_hyperv
        ...
        return false unless product_name.rstrip == 'Virtual Machine'
